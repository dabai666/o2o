...
class SellerController extends AuthController
{
    ...
    public function salestatistics(){
        $args = Input::all();
        $result = $this->requestApi('order.sale_statistics', $args);
        View::share('data', $result['data']['list']);
        View::share('title', '销售统计');
        return $this->display();
    }
    /**
     * 首页信息
...
//        var_dump($result['data']);die;
    ...
    public function goods()
    {
        ...
        $cate = $this->requestApi('Goodscate.getById',$args);
        View::share('cate',$cate['data']);
        if($_GET['status'] != ''){
...
//        var_dump($result['data'][0]['cate']['autoType']);die;
        View::share('goods', $result['data']);
        View::share('status', $args['status']);
...
//        var_dump($args);die;
        ...
        View::share('args', $args);
//        var_dump($this->staffId);die;
        View::share('showData', Cache::get($this->staffId.'_test'));
        return $this->display();
    }
...
//        var_dump($result);die;
    ...
    public function editnewsystem()
    {
        ...
        View::share('showData', Cache::get($this->staffId.'_test_two'));
    ...
    public function getShopBrand(){
        $data = $this->requestApi('seller.getShopBrand');
//        var_dump($data);die;
        if($data['code'] == 0){
    ...
    {
        $data = Input::all();
        Cache::put($this->staffId.'_test', $data, 5);
        if(!Cache::get($this->staffId.'_test')){
            return false;
        }
        return ;
    }

    public function showDataTwo(){
        $data = Input::all();
        Cache::put($this->staffId.'_test_two', $data, 5);
        if(!Cache::get($this->staffId.'_test_two')){
            return false;
        }
        return ;
    }
    ...
    public function shopdetail()
    {
		...
		$share['url'] = str_replace('http://', 'https://', $share['url']);
