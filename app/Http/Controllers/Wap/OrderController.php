...
//		dd($result['data']);
        ...
        if(!empty($activity_result['data'])){
            ...
            $weixin_arrs = $this->requestApi('Useractive.getWeixinUser',array('url' => $url));
        ...
        if($udbType) {
            ...
            if($tid != 0 || $pid != 0 ){
				...
				if(Input::get('detailFlage')){
					View::share('nav_back_url', u('UserCenter/msgshow',['sellerId'=>0]));
				}else{
...
//		dd($result['data']);
        foreach ($result['data'] as $key => $value) {
            if($result['data']['type'] == 1){
                if( (!empty($value['norms']) && $value['norms']['stock'] > 0) || (empty($value['norms']) && $value['goods']['stock'] > 0) )
                {
                    $cartIds_stock[] = $value['id'];
                    $zfee += $value['norms']['price']*$value['num'];
                }
            }else{
                $cartIds_stock[] = $value['id'];
                $zfee += $value['price'] > 0 ? $value['price']*$value['num'] : $value['goods']['price']*$value['num'];
				$result['data'][$key]['price'] = $value['price'] > 0 ? $value['price'] : $value['goods']['price'];
            }
        ...
        $fee= $this->requestApi('order.compute1',['cartIds'=>$ids,'promotionSnId'=>$promotion['data']['id'],'addressId'=>$address['data']['id'],'cancel'=>Input::get('cancel')]);

//        var_dump($fee);die;
		View::share('fee', $fee['data']);
//		var_dump($fee['data']);die;
		...
		if(!empty($cartIds)){
			$nav_back_url = u('GoodsCart/index');
	...
	public function sellerAllowTime($seller) {
		...
		//获取营业时间
		$staffstime = $this->requestApi('staffstime.lists',['id'=>$seller['id']]);
		$deliveryTime = $this->requestApi('seller.deliveryTime',['sellerid' => $seller['id']]);
		if(empty($staffstime['data']))
		{
			return null;
		}
		...
		foreach ($staffstime['data'] as $key => $value) {
			...
			foreach ($value['week'] as $k => $v) {
				...
				//$hoursInt 	= explode(' ', $value['times']);
				$hoursInt 	= explode(' ', $deliveryTime['data']);
				foreach ($hoursInt as $k2 => $v2) {
					...
					foreach ($timeList[$v]['hours'][$k2] as $k3 => $v3) {
						...
						// while ( $beginTime <= $endTime) {
							// $timeList[$v]['time'][] = Time::toDate($beginTime, 'H:i');
							// $beginTime += 120 * 60; //分钟转换成秒
						// }
						while ( $beginTime <= $endTime) {
							$beginTimes = $beginTime;
							$beginTime += 120 * 60; //分钟转换成秒
							if($beginTime > $endTime){
								//$timeList[$v]['time'][] = Time::toDate($beginTimes, 'H:i')." - ".Time::toDate($endTime, 'H:i');
							}else{
								$timeList[$v]['time'][] = Time::toDate($beginTimes, 'H:i')." - ".Time::toDate($beginTime, 'H:i');
							}
						}
					}
					unset($timeList[$v]['hours']);
	...
	public function toOrder() {
	    ...
	    $result = $this->requestApi('order.create1',$args);
	    return Response::json($result);
	}
