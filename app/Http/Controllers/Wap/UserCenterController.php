...
class UserCenterController extends UserAuthController {
    ...
    public function index() {
        ...
        $wap_promotion = $config['wap_promotion'] ? $config['wap_promotion'] : '优惠券';
        $wap_integral = $config['wap_integral'] ? $config['wap_integral'] : '积分';
    ...
    public function collectList($tpl='collect_item') {
		...
		$cart = [];
		foreach($result_cart['data'] as $key => $value){
    ...
    public function addressdetail() {
        ...
        if ((int)Input::get('id') > 0) {
            ...
            if($address_info['cityId'] != Input::get('cityId')){
                unset($address_info['detailAddress']);
                unset($address_info['address']);
                $address_info['cityId'] = Input::get('cityId');
            }
        ...
        //获取当前城市下区县的列表
        $cityName = $data['data'] ? $data['data']['city']['name'] : $address_info['city']['name'];
        $city =  $this->requestApi('user.address.getbyname',['name'=>$cityName]);
        View::share('city', $city['data']['areas']);
...
//            $config['appSecret'] = '4af43812ccce2ff4df1071b931a504c2';
	...
	public function getNorms(){
		$data = Input::all();
		$result = $this->requestApi('collect.norms',$data);
		return Response::json($result);
	}
	
    //会员注册详细信息
	public function detailreg(){
		
        //获取信息
        $detailInfo = $this->requestApi('user.info.getdetail');
        View::share('detailInfo', $detailInfo);
//        if($detailInfo['info'] || $detailInfo['info'] !== null){
//            return Redirect::to(u('UserCenter/index'));
//        }
        if($detailInfo['info']['shoptype']){
            $shoptypeOwn = explode(',',$detailInfo['info']['shoptype']);
            View::share('shoptypeOwn', $shoptypeOwn);
        }
        ...
        View::share('city', $city['data']['areas']);
        //获取门店类型
        $shoptype = $this->requestApi('user.info.getshoptype');  
        View::share('shoptype', $shoptype);
        
        return $this->display(); 
	}
	
    //保存会员注册详细信息
    public function saveDetailreg(){
        $data = Input::all();
        $result = $this->requestApi('user.info.savedetail',$data);
        return Response::json($result);
    }
