...
class GoodsCartController extends AuthController {
	...
	public function index() {
        ...
        $result_cart = $this->requestApi('shopping.listsTwo', ['location'=>$address['data']['mapPointStr'],'cityId'=>$address['data']['cityId']]);
        if(!$bln){
        ...
        foreach($result_cart['data'] as $k => $v){
            $result_cart['data'][$k]['price'] = $result_cart['data'][$k]['salePrice'] > 0 ? $result_cart['data'][$k]['salePrice'] : $result_cart['data'][$k]['price'];
            foreach($v['goods'] as $kk => $vv){
                $result_cart['data'][$k]['price'] = $result_cart['data'][$k]['salePrice'] > 0 ? $result_cart['data'][$k]['salePrice'] : $result_cart['data'][$k]['price'];
                $result_cart['data'][$k]['price'] = $result_cart['data'][$k]['price'] + $vv['processingPrice'] * $vv['num'];
            }
        }
