	...
	public function dologin() {
		...
		if ($result['code'] == 0) {
            ...
            if(FANWEFX_SYSTEM && $config['fx_user_check'] == 0){ //登录
                $args_data['user_id'] = $result['data']['fanweId'];
                $fx_userinfo = $this->requestApi('fx.api', ['path'=>'get_user_info', 'args'=>$args_data]);
                if(empty($fx_userinfo['data'])){ //没有这个用户就 注册啊
                    $args['username'] = $result['data']['mobile'];
                    $args['password'] = $data['pwd'];
                    $args['nickname'] = $result['data']['name'];
                    $args['photo'] = $result['data']['avatar'];
                    $args['mobile'] = $result['data']['mobile'];
                    if(!empty($result['data']['invitationId'])){
                        $invitation_info = $this->requestApi('user.fanwe_id',['userId'=>$result['data']['invitationId']]);
                        if(!empty($invitation_info['data']['fanweId'])){
                            $args['recommender_id'] = $invitation_info['data']['fanweId'];
                        }
                    }
...
//            $return_url = u('UserCenter/detailreg');
//        }
        //跳到分销平台
        $args =  Input::all();
        $goshareping = $args['goshareping'];
...
//                $config['appId'] = 'wxdec1e10223f8e4be';
        ...
        if(empty($is_user['data']['address']) && empty($is_user['data']['id'])){
            ...
            if(!empty($user_info)) {
                ...
                $this->requestApi('user.regweixin',$data);
...
//        }
//
//        return Redirect::to(u('UserCenter/detailreg'));
...
//            $config['appSecret'] = '4af43812ccce2ff4df1071b931a504c2';
    ...
    public function guide(){ 
		...
		//print_r($getWeixinUser['data']);die();
