...
//        var_dump($tpl);die;
    ...
    public function detail1(){
        ...
        if($option['id'] == ONESELF_SELLER_ID){
		 ...
		 if($option['showurl'] == 1){
            $return_url = u('goods/index',$option);
    ...
    public function detail(){
        ...
        if($option['showurl'] == 1){
            $return_url = u('goods/index',$option);
            View::share('nav_back_url',$return_url);
        }else if($option['showurl'] == 2){
            $return_url = u('Tag/index');
            View::share('nav_back_url',$return_url);			
		}
        $protocol = (!empty($_SERVER['HTTPS']) && $_SERVER['HTTPS'] !== 'off' || $_SERVER['SERVER_PORT'] == 443) ? "https://" : "http://";
        $url = $protocol.$_SERVER[HTTP_HOST].$_SERVER[REQUEST_URI];
    ...
    public function shopdetail()
    {
         ...
         * 加字
         */
        $img->text($site_name,   200,420,function ($font) {
            $font->file(base_path() . '/resources/fonts/msyh.ttf');
            $font->size(12);
            $font->color('#000');
            $font->align('bottom');
    ...
    public function authenticate(){
        $args = Input::all();
        $authenticate = $this->requestApi('seller.authenticate',$args);
        View::share('authenticate',$authenticate['data']);
        return $this->display();
    }
