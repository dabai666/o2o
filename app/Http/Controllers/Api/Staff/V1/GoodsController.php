...
//        var_dump(1);die;
        $data = GoodsService::goodsUpdate(
            ...
            $this->request('goodsProcessingCharges'),
            $this->request('brandId'),
            $this->request('price'),
            $this->request('sale_price'),
            $this->request('weight')
        );
        return $this->output($data);
    }
...
//    }
    ...
    public function detail(){
        $sellerId = $this->sellerId;
        $id = (int)$this->request('id');
        $data = Goods::where('id', $id)->where('seller_id', $sellerId)->with('extend','norms','seller','goodsStaff.staffers','systemTagListPid','systemTagListId','goodsProcessingCharges')->first();
        if ($data->id){
            $data = $data->toArray();
            ...
            if($brandId > 0){
                $list = ShopBrand::where('id',$brandId)->first();
