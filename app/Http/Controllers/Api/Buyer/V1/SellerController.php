...
class SellerController extends BaseController {
	 ...
	 * 获取商家收藏状态
	 */
	public function get_conllect(){
		$is_conllect = UserCollect::where('user_id','=',$this->userId)->where('seller_id','=',$this->request('id'))->count();
		return $this->outputData($is_conllect);
	}
    /**
     * 列表
     */
    ...
    public function authenticate(){
        $data = SellerService::getSellerAuthenticate((int)$this->request('id'));
        if(!$data){
            return $this->outputCode(30001);
        }
        return $this->outputData($data);
	...
	public function deliveryTime(){
		$data = SellerService::getDeliveryTime($this->request('sellerid'));
		return $this->outputData($data);
	}
