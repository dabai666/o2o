...
class PromotionController extends BaseController {
    ...
    public function lists(){
        ...
        foreach($data['list'] as $k => $v){
            $data['list'][$k]['promotionSnExpireCount'] = PromotionSn::where('expire_time','<',UTC_TIME)->where('use_time','=',0)->where('promotion_id','=',$v['id'])->count();
            $data['list'][$k]['count'] = $v['promotionSnCount'] - $v['usePromotionSnCount'] - $data['list'][$k]['promotionSnExpireCount'];
        }
