...
//    }
    ...
    public function serviceSave() {
        ...
        if($args['type'] == Goods::SELLER_SERVICE){
            ...
            $norms['ids'] =  $args['_id'] ? $args['_id'] : $args['id'];
    ...
    public function goodsEdit(){
        $args = Input::all();
        $args['status'] = 1;
        $brand = $this->requestApi('ShopBrand.getList',$args);
        if(!count($brand['data'])){
    ...
    public function createGoods(){
        $args = Input::all();
        $brand = $this->requestApi('ShopBrand.getList',array('status'=>1));
        if(!count($brand['data'])){
            $brand['data'][0] = ['id'=>0, 'name'=>'请选择'];
        }
        View::share('brand', $brand['data']);
    ...
    public function systemgoodssave()
    {
        ...
        $norms['prices'] = $args['_price'];
        $norms['stocks'] = $args['_stock'];
        $args['norms'] =json_decode( json_encode($norms),true);
        unset($args['_id'],$args['_name'],$args['_price'],$args['_stock']);
