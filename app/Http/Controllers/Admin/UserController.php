...
class UserController extends AuthController {  
	...
	public function index() {
        ...
        $userTagInfo = $this->requestApi('user.getUserTag');
		$userTagDefaule = [
		    'id' => 0,
            'name' => '所有',
        ];
		array_unshift($userTagInfo,$userTagDefaule);
        $userTag = [];
        foreach($userTagInfo as $key => $val){
            $userTag[$val['id']] = $val['name'];
        }
        View::share('userTagInfo', $userTagInfo);
        View::share('userTag', $userTag);
		return $this->display();
	}
	...
	public function edit() {
		...
		$userTagInfo = $this->requestApi('user.getUserTag');
		$userAddressInfo = $this->requestApi('user.getUserAddressBasic', Input::all());
        /*返回处理*/
		if($result['code'] > 0 ) {
        ...
        $shoptype = explode(',',$result['data']['detail']['shoptype']);
		View::share('data', $result['data']);
		View::share('userTag', $userTag);
		View::share('shoptype', $shoptype);
		View::share('userAddressInfo', $userAddressInfo);
		return $this->display();
	}
	/**
	* 
