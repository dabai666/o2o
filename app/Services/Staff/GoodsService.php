...
class GoodsService extends \YiZan\Services\GoodsService
{
 ...
 //print_r(DB::getQueryLog());exit;
    ...
    public static function goodsCreate($sellerId, $cateId, $name, $images, $norms, $brief, $price, 
        $duration, $staffIds, $stock, $sort = 100, $type, $sellerType, $systemTagListPid, $systemTagListId,$systemGoodsId,$goodsSn="",$goodsProcessingCharges=array(),$brandId=0,$originalPrice=0,$salePrice=0,$weight=0) {
        ...
        try{
            ...
            if($type == \YiZan\Models\Goods::SELLER_GOODS) {
                ...
                foreach ($norms as $key => $value) {
                    ...
                    $goods_norms->sale_price    = $value['sale_price'];
                    $goods_norms->weight        = $value['weight'];
                    $goods_norms->save(); 
                }
                foreach($goodsProcessingCharges as $k => $value){
    ...
    public static function goodsUpdate( $sellerId, $id, $cateId, $name, $images, $norms, $brief, $price, 
        $duration, $staffIds, $stock, $systemTagListPid, $systemTagListId,$systemGoodsId,$goodsSn,$goodsProcessingCharges=array(),$brandId=0,$originalPrice=0,$salePrice=0,$weight=0) {
        $result = array(
            'code'  => self::SUCCESS,
            'data'  => null,
            'msg'   => ''
        ...
        if ($id < 1) {
            return self::goodsCreate($sellerId, $cateId, $name, $images, $norms, $brief, $price, $duration, $staffIds, $stock, 100, $type, $sellerType, $systemTagListPid, $systemTagListId,$systemGoodsId,$goodsSn,$goodsProcessingCharges,$brandId,$originalPrice,$salePrice,$weight);//, $deductType(提成方式1固定2百分比'), $deductVal('提成值')
        } else {
            if($sellerType != \YiZan\Models\Seller::SELF_ORGANIZATION && $type == \YiZan\Models\Goods::SELLER_SERVICE)
            {
                $staffs = $staffIds[0];
                   ...
                   // $norms = json_decode($norms, true); 
                    ...
                    foreach ($norms as $key => $value) {          
                        ...
                        if($norms_item){
                            ...
                            $norms_item->sale_price = $value['sale_price'];
                            $norms_item->weight = $value['weight'];
                            $norms_item->save();
                        } else {
                            $goods_norms = new GoodsNorms();
                            $goods_norms->seller_id     = $sellerId;
                    ...
                    //删除不在编辑列表中的规格
                    if($goodsProcessingCharges){
                        foreach ($goodsProcessingCharges as $key => $value) {
                            $nidss[] = $value['id'];
                        }
                        if(!empty($nidss)){
                            GoodsProcessingCharges::whereNotIn('id',$nidss)
                                ->where('seller_id', $sellerId)
                                ->where('goods_id', $id)
                                ->delete();
                        }
                    } else {
                        GoodsProcessingCharges::where('seller_id', $sellerId)
                            ->where('goods_id', $id)
                            ->delete();
                    }
                    if($goodsProcessingCharges){
                        foreach($goodsProcessingCharges as $k => $v){
