    ...
    public static function dispose($adminId, $id, $content, $status) {
        ...
        try {
            ...
            if($status == STATUS_WITHDRAW_REFUSE)
            {
                ...
                \YiZan\Services\SellerMoneyLogService::createUserLog($withdraw->user_id,10, $withdraw->money, "提现失败退回账户",1,$withdraw->id);
            }
            if($extend->lock_amount > 0){
                $extend->lock_amount  -=  $withdraw->money;
            }
            $extend->save();
