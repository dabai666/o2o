...
class SellerService extends \YiZan\Services\SellerService {
     ...
     * @return array          订单列表
     */
    public static function getSystemList($mobileName, $mobile, $provinceId, $cityId, $areaId, $status,$cateId, $notIds, $page, $pageSize, $type=0, $flage=false)
    {
        ...
        /*if($type && (in_array($type,array(6,7)))){
            $list->where('store_type','=',0);
        }*/
        if($flage){
            $list->where('store_type','=',0);
        }
     ...
     * @return [type]      		[description]
     */
    public static function activityLists($ids,$activityId='',$type='') {
        $list = Seller::where('seller.id', '<>', ONESELF_SELLER_ID)->whereIn('seller.id', $ids)->orderBy('seller.id', 'desc');

        $list = $list->where('seller.is_del', 0)->where('seller.is_check', 1)->whereIn('seller.type', [1,2]);
        if($type){//处理促销和秒杀
            $lists = array();
            if($activityId){
                $list = $list
                    ->leftJoin('activity_seller','activity_seller.seller_id','=','seller.id')
                    ->where('activity_seller.activity_id','=',$activityId)
                    ->leftJoin('goods','goods.id','=','activity_seller.goods_id')
                ;
                $filed = array('seller.id','seller.name','activity_seller.goods_id','goods.total_stock');
                $list = $list->select($filed)->get()->toArray();
                foreach($list as $k => $v){
                    $lists[$v['id']]['name'] = $v['name'];
                    $lists[$v['id']]['totalStock'] = $v['totalStock'];
                }
                unset($list);
            }else{
                $filed = array('seller.id','seller.name');
                $list = $list->select($filed)->get();
                $list = $list ? $list->toArray() : array();
                if($type == 8){
                    $activity = Session::get('Activity.sale');
                    $form = $activity['form'];
                }else{
                    $activity = Session::get('Activity.kill');
//                    var_dump($activity);die;
                    $form = $activity['form'];
                }
                foreach($list as $k => $v){
                    $lists[$v['id']]['name'] = $v['name'];
                    $lists[$v['id']]['totalStock'] = $form['stocks['.$v['id'].']'];
                }
            }
            return $lists;
        }else{
            $list  = $list->lists('seller.name','seller.id');
        }
        return $list;
    }
...
//            $data['map_pos_str'] = null;
    ...
    public static function getBusinessStatisticsList($name, $month, $year, $cityId, $page, $pageSize){
        ...
        if($name){
            $sellerIds = Seller::where('mobile', '=', $name)->where('id','<>',ONESELF_SELLER_ID)->lists('id');
