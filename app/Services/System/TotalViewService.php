...
class TotalViewService extends \YiZan\Services\BaseService 
{
    ...
    public static function total() {
        ...
        $data['withdraw'] = SellerWithdrawMoney::where('status', '0')->where('seller_id' ,'>',0)->count(); //待审提现
        $data['userWithdraw'] = SellerWithdrawMoney::where('status', '0')->where('user_id' ,'>',0)->count(); //待审提现
