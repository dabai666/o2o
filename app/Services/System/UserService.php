...
class UserService extends \YiZan\Services\UserService {
	 ...
	 */
	public static function getLists($mobile, $name, $status, $userType, $userTag, $page, $pageSize) {
		$list = User::with('regProvince', 'regCity', 'loginProvince', 'loginCity', 'seller', 'staff','bank');
        $list = $list->leftJoin('user_detail','user_detail.user_id','=','user.id');
        $list = $list->select('user.*','user_detail.shoptype');


		if (!empty($name)) {//搜索名称
			$list->where('user.name' ,'like', '%'.$name.'%');
		}
		...
		if (!empty($mobile)) {//搜索手机号
			if(strlen($mobile) == '11')
				...
				$list->where('user.mobile','like', '%'.$mobile.'%');
		}

		if ($status > 0) {//状态
			$list->where('user.status', $status - 1);
		}

        if ($userType > 0) {
            if ($userType == 1) {
                $list->whereIn('user.id', function($query) {
                ...
                $list->whereNotIn('user.id', function($query) {
                    $query->select('user_id')->from('seller');
                })
                    ->whereNotIn('user.id', function($query) {
                        $query->select('user_id')->from('seller_staff');
                    });
            }
        }
        if($userTag > 0){
            $list = $list->where('user_detail.shoptype','like', '%,'.$userTag);
            $list = $list->orWhere('user_detail.shoptype','like', $userTag.',%');
        }
		$total_count = $list->count();
		$list->orderBy('user.id', 'desc');
	...
	public static function getById($id) {
		return User::with('regProvince', 'regCity', 'loginProvince', 'loginCity','bank','detail')->find($id);
	}
...
//        print_r(DB::getQueryLog());exit;
    ...
    //获取会员注册时的地址
    public function getUserAddressBasic($userId){
        $address = UserAddress::with('province','city','area')
