...
class StatisticsService extends \YiZan\Services\BaseService {  
	...
	public static function getClassifyListDetail($page,$pageSize,$systemTagListId,$month,$year){
		$page = $page ? intval($page) : 1;
		$pageSize = $pageSize ? intval($pageSize) : 20;
		$db_prefix = env('DB_PREFIX');
		//二级标签名称 商品名称 分类名称 品牌名称
//		DB::connection()->enableQueryLog();

		$month = $month ? ltrim($month,0) : Time::toDate(UTC_TIME,'m');
		if($month < 10){
			$month = "0".$month;
		}
		$year = $year ? $year : Time::toDate(UTC_TIME,'Y');
		...
		$select = "{$db_prefix}order_goods.goods_id,{$db_prefix}system_tag_list.name as tagListName,{$db_prefix}order_goods.goods_name,{$db_prefix}goods_cate.name,{$db_prefix}shop_brand.name as brandName,sum({$db_prefix}order_goods.num) as totalNum,sum({$db_prefix}order_goods.num * {$db_prefix}goods_norms.weight) as totalWeight,sum({$db_prefix}order_goods.num * {$db_prefix}order_goods.price) as totalPrice";
		$ordergoods = OrderGoods::select(DB::raw($select))
				...
				->leftJoin('system_tag_list','system_tag_list.id','=','goods.system_tag_list_id')
				->leftJoin('shop_brand','shop_brand.id','=','goods.brand_id')
...
//							->where('seller_id',ONESELF_SELLER_ID);
		...
		if($systemTagListId){
			$total = $total->where('system_tag_list.id','=',$systemTagListId);
			$ordergoods = $ordergoods->where('system_tag_list.id','=',$systemTagListId);
		}
		$totalCount = $total->groupBy('order_goods.goods_id')->count('order_goods.goods_id');
