...
class OrderService extends \YiZan\Services\OrderService {
        ...
        if($payment == 1)
        {
            ...
            foreach ($carts as $key => $value) {
                if( in_array($value['goodsId'], $specialGoodsIds) )
                {
                    ...
                    {
                        //统计优惠的特价金额
                        if(!empty($value['norms']))
                        {
                            $totalSalePrice = $value['norms']['price'] * $value['num'] * (1-($activityGoodsInfo['sale']/10)); //1 -
                        }
                        else
                        {
                        ...
                        foreach ($goodsData as $k => $v) {
                            ...
                            if( !empty($v['goods_norms']) && $v['goods_norms_id'] == $value['norms']['id']  && $v['goods_id'] == $value['goodsId'] )
                            {
                                $goodsData[$k]['price'] = $value['norms']['price'];//$activityGoodsInfo['price']; //重写原价
                                $goodsData[$k]['sale_price'] = $value['norms']['price'] * $value['num'] * ($activityGoodsInfo['sale']/10);
                            }
                        }
 ...
 //如果总额小于首单减的金额 修改首单减金额
    ...
    public static function create1($userId, $cartIds, $addressId, $giftContent, $invoiceTitle, $buyRemark, $appTime, $payment, $promotionSnId, $freType, $orderType, $sendWay, $isUseIntegral, $detailAddress, $mapPoint, $provinceId, $cityId, $areaId, $name, $mobile, $doorplate, $isSaveAddress, $storeType ) {
        ...
        $carts = ShoppingCart::where('user_id', $userId)
            ->whereIn('id', $cartIds)
            ->with('goods', 'norms', 'goodsProcessingCharges')
            ->get()->toArray();
        //有不存在的购物车信息
        if (count($cartIds) != count($carts)) {
            $result['code'] = 60502;
        ...
        if($appTime == 0){
			...
			$appTimes = explode(' - ',$appTime);
            $appTime = Time::toTime($appTimes[0]);
            if ($appTime < UTC_TIME) {
                $result['code'] = 60508;
                return $result;
            }
        ...
        foreach ($carts as $key => $val) {
            ...
            $activitySel = ActivitySeller::where('goods_id','=',$val['goods']['id'])->first();
            ...
            $duration = $val['goods']['unit'] == 1 ? $val['goods']['duration'] * 60 : $val['goods']['duration'];
            if($val['normsId'] > 0){
                $normsPrice = $val['norms']['salePrice'] > 0 ? $val['norms']['salePrice'] : $val['norms']['price'];
            }else{
                $normsPrice = $val['goods']['salePrice'] > 0 ? $val['goods']['salePrice'] : $val['goods']['price'];
            }
            $goodsData[$key] = [
                'seller_id' => $val['sellerId'],
                'goods_name' => $val['goods']['name'],
                ...
                'num' => $val['num'],
                'sale_price' => 0.00,
                'process_id' => $val['goodsProcessingCharges']['id'],
                'process_name' => $val['goodsProcessingCharges']['name'],
                'process_price' => $val['goodsProcessingCharges']['price'],
            ];

            $goodsFee += ($goodsData[$key]['price'] * $val['num']) + ($goodsData[$key]['process_price'] * $val['num']);
            $num += $val['num'];
            $totalDuration += $duration * $val['num'];
            //$appTime = $val['goods']['type'] == 2 ? $val['serviceTime'] : $appTime;
            $orderType = $val['goods']['type'];
        ...
        if($payment == 1)
        {
            ...
            if(!$firstOrder && !empty($activity['new']))
            {
                $activityFirstId = $activity['new']['id'];
                $firstOrderCutMoney = $activity['new']['cutMoney'];
            }
        ...
        $orderData  = [
            ...
            'integral' => '0',
            'integral_fee' => '0.00',
            'activity_first_id' => $activityFirstId,
            'activity_full_id' => $activityFullId,
            'activity_goods_id' => $activityGoodsId,
            'system_full_subsidy' => $systemFullSubsidy,
            'seller_full_subsidy' => $sellerFullSubsidy,
     ...
     */
    public static function orderCompute($userId, $cartIds, $promotionSnId, $addressId,$cancel){
        ...
        foreach ($data as $key => $value) {
            if( in_array($value['goodsId'], $specialGoodsIds) )
            {
                ...
                if($activityCount < $activityGoodsInfo['joinNumber'])
                {
                    ...
                    if(!empty($value['norms']))
                    {
                        $specialOrderCutMoney += $value['norms']['price'] * $value['num'] * (1-($activityGoodsInfo['sale']/10));//1 -
                    }
                    else
                    {
    ...
    public static function orderCompute1($userId, $cartIds, $promotionSnId, $addressId,$cancel){
...
//        var_dump($data);die;
        ...
        if(!IS_OPEN_FX){
            ...
            foreach ($data as $key => $value) {
                if($value['norms']){
                    $norms_price = $value['norms']['SalePrice'] ? $value['norms']['SalePrice'] : $value['norms']['price'];
                    $goodsFee += $norms_price * $value['num'];
                } else {
                    $goods_price = $value['salePrice'] > 0 ? $value['salePrice'] : $value['price'];
                    $goods_price = $goods_price > 0 ? $goods_price : $value['goods']['price'];
                    $goodsFee += $goods_price * $value['num'];
                }
                if(isset($value['goodsProcessingCharges'])){
                    $goodsFee += $value['goodsProcessingCharges']['price'] * $value['num'];
                }
            }
        }else{
            if($data['shareUserId'] <= 0){
                ...
                foreach ($data as $key => $value) {
                    ...
                    if($value['norms']){
                        //$value['norms']['salePrice'] ? $value['norms']['salePrice'] :
                        $norms_price =  $value['norms']['price'];
                        $goodsFee += $norms_price * $value['num'];
                    } else {
        ...
        $processFee = 0;
        foreach($data as $v){
            if($v['goodsProcessingCharges'])
            $processFee += $v['goodsProcessingCharges']['price'] * $v['num'];
        }

        //总金额 = 商品金额 + 加工费 + 配送费 - 优惠金额 - 首单立减 - 满减优惠 - 商品特价
        /*var_dump($goodsFee , $freight , $processFee , $discountFee , $firstOrderCutMoney , $fullOrderCutMoney , $specialOrderCutMoney);
        die();+ $processFee*/
        $totalFee = $goodsFee + $freight - $discountFee - $firstOrderCutMoney - $fullOrderCutMoney - $specialOrderCutMoney;
        $totalFee = $totalFee > 0 ? $totalFee : 0;
            ...
            if($invitation->id  > 0 ){
                ...
                if($money >= $invitation->full_money){
                    ...
                    if(strtolower($user->invitation_type) == "seller"){
                        ...
                        $money =  $order->goods_fee - $order->freight;
                        ...
                        if($storeTtype == 1 && IS_OPEN_FX){
                            if($firstInvitation){
                                ...
                                if(strtolower($secondInvitation->invitation_type) == "user" || $secondInvitation->invitation_id > 0){
                                    ...
                                    //$thirdInvitation = User::find($secondInvitation->invitation_id);
                                }
                                //四级
                                /*if(strtolower($thirdInvitation->invitation_type) == "user" || $thirdInvitation->invitation_id > 0) {
                                    ...
                                    $datas[] = [
                                        ...
                                        'order_id'          => $order->id,
                                        'user_id'           => $order->user_id,
                                        'invitation_type'   => strtolower($thirdInvitation->invitation_type),
                                        'invitation_id'     => $thirdInvitation->invitation_id,
                                        'percent'           => $invitation->is_all_user_percent_third,
                                        'return_fee'        => $thirdReturnMoney,
                                        'level'             => 4,
                                        'create_time'       => UTC_TIME,
                            ...
                            if($firstInvitation){
                                ...
                                //写入扣款日志
                                if(!InvitationService::createLog($first_data)){
                                ...
                                if(strtolower($firstInvitation->invitation_type) == "user" || $firstInvitation->invitation_id > 0){
                                    ...
                                    //写入扣款日志
                                    if(!InvitationService::createLog($second_data)){
                                        throw new Exception("insert error!", 1);
                                    }
                                    //3级邀请人只有是会员才发放返现
                                    if(strtolower($secondInvitation->invitation_type) == "user" || $secondInvitation->invitation_id > 0){
                                        ...
                                        //写入扣款日志
                                        if(!InvitationService::createLog($third_data)){
                                            throw new Exception("insert error!", 1);
                                        }
                                    }
                                }
