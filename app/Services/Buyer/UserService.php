...
class UserService extends \YiZan\Services\UserService {
    ...
    public static function payCharge($userId, $payment, $extend = [], $money) {
        ...
        $payLog = basePaymentService::createPayLog($money, $payment, $extend, $userId);
    ...
    public static function getAccount($userId){
        ...
        $data['lockAmount'] = $result->lock_amount;
     ...
     * 微信手机号绑定
     */
    public function bindmobile($userId,$unionid,$openid,$mobile,$verifyCode){
        $result = array(
            'code'  => 0,
            'data'  => null,
            'msg'   => ''
    ...
    public function moveunionid($userId,$unionid,$openid,$mobile){
        $result = array(
            'code'  => 0,
            'data'  => null,
            'msg'   => ''
        ...
        DB::beginTransaction();
        try {
            User::where('id',$xuser->id)->update(['openid'=>$openid,'unionid'=>$unionid]);
