...
//            S.province_id,S.city_id,S.area_id,
	...
	//获取商户配送时间
	public static function getDeliveryTime($sellerid){
		$time =  SellerDeliveryTime::where('seller_id', $sellerid)->get()->toArray();
        foreach ($time as $k => $v) {
            $stimes[] = $v['stime'] . '-' . $v['etime'];
        }
        $deliveryTime = $stimes ? implode(',', $stimes) : '00:00-24:00';
		return $deliveryTime;
	}

	//获取商家资质证明
    public static function getSellerAuthenticate($id){
        $data = SellerAuthenticate::where('seller_id',$id)->first();
        return $data;
    }
