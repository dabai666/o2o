...
class CollectService extends \YiZan\Services\CollectService
{
    ...
    public static function goodsList($userId, $type, $page)
    {
        ...
        $list = $list->orderBy('id', 'desc')
            ->skip(($page - 1) * 20)
            ->with('seller','goods','seller.extend','goods.extend','goods.norms','goods.goodsProcessingCharges')
            ->take(20)
            ->get()
            ->toArray();
        ...
        foreach ($list as $key => $value) {
            if ($type == 2) {
                ...
                if($value['goods'] != ""){
                    ...
                    $lists[$key]['norms'] = $value['goods']['norms'];
                    $lists[$key]['goodsProcessingCharges'] = $value['goods']['goodsProcessingCharges'];
                    $lists[$key]['sellerId'] = $value['goods']['sellerId'];
                }
            }
	...
	//获取商品规格
	public static function goodsNorms($goodsid){
		$norms = GoodsNorms::where('goods_id', $goodsid)->get();
		$goodsProcessingCharges = GoodsProcessingCharges::where('goods_id', $goodsid)->get();
		$result['norms'] = $norms;
		$result['goodsProcessingCharges'] = $goodsProcessingCharges;
		return $result;
	}
