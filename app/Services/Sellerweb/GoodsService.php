...
use YiZan\Models\GoodsProcessingCharges;
...
class GoodsService extends \YiZan\Services\GoodsService
{
    ...
    public function getShopBrand(){
        return ShopBrand::where('status',1)->get()->toArray();
    }
	/**
     * 服务/商品列表
	...
	public static function getSystemList($sellerId, $type, $name, $cateId, $status, $notIds, $page, $pageSize) {
        $list = Goods::orderBy('status','desc')
                    ->orderBy('id','desc')
                    ->orderBy('sort','asc')
     ...
     * @return array   创建结果
     */
    public static function systemCreate($sellerId, $sellerType, $type, $staffIds, $name, $price, 
        $cateId, $brief, $images, $duration, $unit, $stock, $buyLimit, $norms, $status, $sort = 100, $deductType, $deductVal = 0, $systemTagListPid, $systemTagListId,$systemGoodsId,$goodsSn,$brandId=0,$originalPrice=0,$salePrice=0,$weight=0,$name1=array(),$price1=array(),$model,$goodsUnit) {
        $result = array(
			'code'	=> self::SUCCESS,
			'data'	=> null,
			'msg'	=> ''
        ...
        try{
            ...
            $goods->sale_price = $salePrice;
            $goods->weight = $weight;
            $goods->brand_id = $brandId;
            $goods->model = $model;
            $goods->goods_unit = $goodsUnit;
            ...
            if($name1 || $price1){
                foreach ($name1 as $key => $value) {
                    $goods_norms = new GoodsProcessingCharges();
//                    $goods_norms = new GoodsNorms();
                    $goods_norms->seller_id     = $sellerId;
                    $goods_norms->goods_id      = $id;
                    $goods_norms->name          = $value;
                    $goods_norms->price         = $price1[$key];
//                    $goods_norms->stock         = $norms['stocks'][$key];
            ...
            if($type == Goods::SELLER_GOODS) {
                ...
                if($norms['names'] || $norms['prices'] || $norms['stocks'])
                {

                    if (count($norms['names']) != count(array_unique($norms['names']))) {
                        $result['code'] = 88885;
                        return $result;
                    }

                    foreach ($norms['names'] as $keys => $values) {
                        if(str_replace(' ','',$values) == ''){
                            $result['code'] = 11000;
                            return $result;
                        }
                    }
                    foreach ($norms['prices'] as $keys => $values) {
                        if(str_replace(' ','',$values) == ''){
                            $result['code'] = 11001;
                            return $result;
                        }
                        if(!preg_match('/^[0-9]+\.?[0-9]{0,2}$/', $values)){
                            $result['code'] = 11003;
                            return $result;
                        }
                        if(sprintf("%.2f", $values) < 0){
                            $result['code'] = 11003;
                            return $result;
                        }
                    }
                    foreach ($norms['stocks'] as $keys => $values) {
                        if($values == ''){
                            $result['code'] = 11002;
                            return $result;
                        }
                ...
                foreach ($norms['names'] as $key => $value) {
                    ...
                    $goods_norms->stock         = $norms['stocks'][$key];
                    $goods_norms->sale_price    = $norms['sale_price'][$key];
                    $goods_norms->weight        = $norms['weight'][$key];
                    $goods_norms->save(); 
                }
            } else {
    ...
    public static function systemUpdate($id, $sellerId, $sellerType, $type, $staffIds, $name, $price, 
        $cateId, $brief, $images, $duration, $unit, $stock, $buyLimit, $norms, $status, $sort = 100, $deductType, $deductVal = 0, $systemTagListPid, $systemTagListId,$brandId=0,$originalPrice=0,$salePrice=0,$weight=0,$name1=array(),$price1=array(),$id1=array(),$model,$goodsUnit) {
        ...
        try{ 
            ...
            if($type == Goods::SELLER_GOODS) {
                ...
                //更新加工费用
                foreach($name1 as $key => $value){
                    $norms_item = GoodsProcessingCharges::where('seller_id', $sellerId)
                        ->where('goods_id', $id)
                        ->where('id', $id1[$key])
                        ->first();
                    if($norms_item){
                        $norms_item->name           = $value;
                        $norms_item->price          = $price1[$key];
                        $norms_item->save();
                    } else {
                        $goods_norms = new GoodsProcessingCharges();
                        $goods_norms->seller_id     = $sellerId;
...
//                        $goods_norms->stock         = $norms['stocks'][$key];
                ...
                foreach ($norms['names'] as $key => $value) {
                    ...
                    if($norms_item){
                        ...
                        $norms_item->sale_price     = $norms['sale_price'][$key];
                        $norms_item->weight         = $norms['weight'][$key];
                        $norms_item->save();
                    } else {
                        $goods_norms = new GoodsNorms();
                        $goods_norms->seller_id     = $sellerId;
    ...
    public static function getGoodsById($sellerId, $id) { 
        $goods = Goods::where('id', $id)
                    ...
                    ->with(['seller', 'cate', 'norms'=>function($query) use($sellerId) {
                                ...
                                },'goodsProcessingCharges'=>function($query1) use($sellerId){
    ...
    //商品名称检测重复
    public static function nameTest($name,$sellerid){
        $goods = Goods::where('name','like','%'.$name.'%')->where('seller_id', $sellerid)->where('auto_type','0')->get()->toArray();
        if($goods){
            $result['code'] = 0;
            $result['data'] = $goods;
        }else{
