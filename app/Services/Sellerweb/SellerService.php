...
class SellerService extends \YiZan\Services\SellerService
{
    ...
    public static function createSeller($sellerType, $mobile, $verifyCode, $pwd, $name, $avatar, $serviceTel, $districtId, $provinceId, $cityId, $areaId, $idcardSn, $idcardPositiveImg, $idcardNegativeImg, $certificateImg, $businessLicenceImg, $address, $mapPos, $mapPoint, $cateIds, $contacts, $type = 'reg',$storeType, $refundAddress) {
        ...
        $pwd = strval($pwd);
        $mapPoint = '26.074508,119.296494';//$sellerType == 3 ? : $mapPoint
        $result = array(
            'code'  => 0,
            'data'  => null,
...
//            'mapPos'                => $mapPos,
        ...
        try {
            ...
            $seller->map_point_str    = $mapPoint ? $mapPoint : '';
            $seller->map_pos_str      = $mapPos["str"] ? $mapPos["str"] : '';
            $seller->map_point        =  $mapPoint ? DB::raw("GeomFromText('POINT(" . str_replace(',', ' ', $mapPoint) . ")')") : DB::raw("GeomFromText('POINT(" . str_replace(',', ' ', '') . ")')");
            ...
            $sellerMap->map_point       =$mapPoint ? DB::raw("GeomFromText('POINT(" . str_replace(',', ' ', $mapPoint) . ")')") : DB::raw("GeomFromText('POINT(" . str_replace(',', ' ', '') . ")')");
    ...
    public static function saveCert($sellerId, $authenticate)
    {
        ...
        $hygieneImg = $authenticate['hygieneImg'];
        ...
        $hygieneImg = self::moveSellerImage($seller->id, $hygieneImg);
        if (!$hygieneImg) {//转移图片失败
            $result['code'] = 10204;
            return $result;
        }
