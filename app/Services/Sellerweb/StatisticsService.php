...
class StatisticsService extends \YiZan\Services\BaseService {
    ...
    public function sale_lists($sellerId, $page, $pageSize){
        $list = GoodsCate::select(DB::raw('sum('.env('DB_PREFIX').'order_goods.num) as totalNum,'.env('DB_PREFIX').'goods_cate.*'))//DB::raw('order_goods.num,sum(order_goods.num) as totalNum')
...
//            ->with('goods')
            ->where('goods_cate.seller_id','=',$sellerId)
...
//            ->orderBy(DB::raw('sum('.env('DB_PREFIX').'order_goods.num)'),'DESC')
            ->count();*/
        return array('list'=>$list,'totalCount'=>count($list));
    }
    /**
     * 营业额
     * @param  $sellerId
     ...
     * @param  $type
     */
    public function goodsreport1($sellerId,$beginDate,$endDate,$type=0,$numOrder=0,$priceOrder=0,$page=1,$pageSize=20){
        ...
        $filed = explode(',',$filed);
        $list = Goods::select($filed)->where('goods.seller_id','=',$sellerId);
        if($beginDate){
            $list = $list->where('create_time','>=',Time::toTime($beginDate));
        }
        if($endDate){
            $list = $list->where('create_time','<',Time::toTime($endDate));
        }
        $list = $list->where('status','=',1)
            ...
            ->take($pageSize)
            ->get();
        $totalCount = Goods::select('goods.id')->where('goods.seller_id','=',$sellerId);
        if($beginDate){
            $totalCount = $totalCount->where('create_time','>=',Time::toTime($beginDate));
        }
        if($endDate){
            $totalCount = $totalCount->where('create_time','<',Time::toTime($endDate));
        }
