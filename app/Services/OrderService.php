...
class OrderService extends BaseService
{
    ...
    public function getVal($userId,$goodsId,$normsId,$processId){
        $result = array(
            'code'	=> 0,
            'data'	=> null,
            'msg'	=> ''
        );
        $data = ShoppingCart::where('user_id',(int)$userId)->where('goods_id',(int)$goodsId)->where('norms_id',(int)$normsId)->where('process_id',(int)$processId)->sum('num');
...
//                ]);//设为系统到期确认
    ...
    public static function confirmOrder($userId, $orderId) {
        ...
        $order->seller_withdraw_time = UTC_TIME + $withdrawDay * 24 * 3600;
            ...
            try{
                foreach ($logs as $log) {
                    if($log->id > 0){
                        ...
                        if($type == 'seller'){
							$sellerIdItem = Seller::where('id',$id)->pluck('id');							
							if($sellerIdItem <= 0){
								continue;
								//$sellerIdItem = Seller::where('user_id',$id)->pluck('id');
							}
                            $sellerMoneyLog = new SellerMoneyLog;
                            $sellerMoneyLog->sn          = Helper::getSn();
                            $sellerMoneyLog->seller_id   = $sellerIdItem;
                            $sellerMoneyLog->type        = SellerMoneyLog::TYPE_INVITATION_BACK;
                            $sellerMoneyLog->related_id  = $orderId;
                            $sellerMoneyLog->money       = $returnFee;
                            $sellerMoneyLog->content     = '订单'.$order['sn'].'结算邀请返现 如余额';
                            $sellerMoneyLog->create_time = UTC_TIME;
                            $sellerMoneyLog->create_day  = UTC_DAY;
                            $sellerMoneyLog->balance     = SellerExtend::where('seller_id',$sellerIdItem)->pluck('money');
                            $sellerMoneyLog->status      = 1;
                            $sellerMoneyLog->save();
                            //增加商家金额
                            SellerExtend::where('seller_id', $sellerIdItem)->increment('money', abs($returnFee));
                        }else if($type == 'user'){
                            $userPayLog = new UserPayLog;
        ...
        $order = Order::select('order.user_id', 'order.id', 'order.pay_type')
            ...
            ->whereNotExists(function($query)
            {
                $query->select(DB::raw(1))
                    ->from('invitation_back_log')
                    ->whereRaw(env('DB_PREFIX') . 'order.user_id = ' . env('DB_PREFIX') . 'invitation_back_log.user_id');
            })
            ->get()
            ->toArray();
    ...
    public function autopei($id){
        ...
        $sql = "SELECT ss.id, COUNT(o.id) AS getorder_num FROM {$tablePrefix}seller_staff AS ss
                ...
                WHERE ss.is_system = 1 AND ss.status = 1 AND ST_Contains(ss.map_pos, GeomFromText ('Point(".$mapPoint.")')) GROUP BY ss.id
