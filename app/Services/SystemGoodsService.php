...
//				'systemTagListId' => $systemTagListId,
        ...
        $count = SystemGoods::where('type', 1)->where('status',1);
		if($systemTagListPid > 0){
		    $count = $count->where('system_tag_list_pid',$systemTagListPid);
        }
		if($systemTagListId > 0){
            $count = $count->where('system_tag_list_id',$systemTagListId);
        }
        if($ids){
...
//				'systemTagListId' => $systemTagListId,
        ...
        $list = SystemGoods::where('type', 1)->where('status',1);
        if($systemTagListPid > 0){
            $list = $list->where('system_tag_list_pid',$systemTagListPid);
        }
        if($systemTagListId > 0){
            $list = $list->where('system_tag_list_id',$systemTagListId);
        }
        if($ids){
            $list->whereIn('id',$ids);
        }
		...
		try{
			...
			foreach ($data as $v) {
				...
				$new_g[] = [
                    ...
                    'weight' => $v['weight'],
                    'process_money' => $v['processMoney']
				];
			}
            Goods::insert($new_g);
            $goodslist = Goods::with("sysNorms")->where('is_updata', 1)->where('seller_id', $sellerId)->get()->toArray();
            $gn = false;
     ...
     * @param  [type] $type      服务类型
     */
    public static function getSellerList($page, $pageSize, $name = '',$type,$status,$systemTagListPid,$systemTagListId,$brandid) {
    ...
    public static function systemUpdate($id, $sellerId, $type, $name, $price,$brief, $images, $duration, $unit, $stock, $buyLimit, $norms, $status, $sort = 100, $systemTagListPid, $systemTagListId,$brandid,$weight) {
        $result = array(
            'code'  => self::SUCCESS,
            'data'  => null,
            'msg'   => ''
        ...
        $system_goods->brandid  = $brandid;
        $system_goods->weight  = $weight;
        ...
        try{
                ...
                if($type == Goods::SELLER_GOODS) {
                    ...
                    //删除不在编辑列表中的规格
                    if(!empty($norms['ids'])){
                        SystemGoodsNorms::whereNotIn('id',$norms['ids'])
                            ->where('system_goods_id', $id)
                            ->delete();
                    } else {
                        SystemGoodsNorms::where('system_goods_id', $id) ->delete();
                    }
                    if($norms['name'] || $norms['price'] || $norms['stock'])
                    {
                        if (count($norms['name']) != count(array_unique($norms['name']))) {
                            $result['code'] = 88885;
                            return $result;
                        }
                        foreach ($norms['name'] as $keys => $values) {
                            if($values == ''){
                                $result['code'] = 88889;
                                return $result;
                            }
                        }
                        foreach ($norms['price'] as $keys => $values) {
                        ...
                        foreach ($norms['stock'] as $keys => $values) {
                            if($values == ''){
                                $result['code'] = 88888;
                                return $result;
                            }
                    ...
                    foreach ($norms['name'] as $key => $value) {
                        $norms_item = SystemGoodsNorms::where('system_goods_id', $id)
                            ->where('id', $norms['ids'][$key])
                            ->first();
                        if($norms_item){
                            $norms_item->name           = $value;
                            $norms_item->price          = $norms['price'][$key];
                            $norms_item->stock          = $norms['stock'][$key];
                            $norms_item->save();
                        } else {
                            $data['system_goods_id']   = $system_goods->id;
                            $data['name']  = $value;
                            $data['price'] = $norms['price'][$key];
                            $data['stock'] = $norms['stock'][$key];
                            SystemGoodsNorms::insert($data);
                        }
                    }
                }
               DB::commit();
