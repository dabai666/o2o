	...
	public static function getLists($sn, $promotionId, $status,$actName,$mobile,$beginTime,$endTime, $actType, $page, $pageSize) {
		...
		if ($status > 0) {
            if ($status == 1) {
                ...
                $list->where('use_time',0)->where('expire_time','>=',UTC_TIME);
            } elseif ($status == 3) {
                $list->where('use_time', '>', 0);
            } elseif($status == 5){
                $list->where('use_time',0)->where('expire_time','<',UTC_TIME);
            }elseif($status == 4){
                /*$orderPro = OrderPromotion::whereIn('order_id',function($query){
                    $query->select('id')
                        ->from('order')
                        ->whereNotNull('cancel_time');
                })->get();
                $orderPro = $orderPro ? $orderPro->toArray() : array();
                $promotionsnId = array();
                foreach($orderPro as $k => $v){
                    $promotionsnId[] = $v['promotionSnId'];
                }
                $list->whereIn('id',$promotionsnId);*/
                $list = $list->whereIn('id', function($query){
                    $query->select('promotion_sn_id')
                        ->from('order_promotion')
                        ->whereIn('order_id', function($query1){
                        $query1->select('id')->from('order')->whereNotNull('cancel_time');
                    });
                });
            }else {
