...
class GoodsService extends \YiZan\Services\GoodsService {
     ...
     * @return array   创建结果
     */
    public static function systemSave( $sellerId, $sellerType, $type, $staffIds, $name, $price, $cateId, $brief, $images, $duration, $unit, $stock, $buyLimit, $norms, $status, $sort = 100, $systemTagListPid, $systemTagListId,$systemGoodsId,$goodsSn) {
...
//                $goods->deduct_val      = $deductVal;
            ...
            if($type == Goods::SELLER_GOODS) {
                ...
                if($norms['name'] || $norms['price'] || $norms['stock'] || $norms['weight'])
                {
                    ...
                    /*foreach ($norms['weight'] as $keys => $values){
                        if($values == ''){
                ...
                foreach ($norms['name'] as $key => $value) {
                    ...
                    $goods_norms->price         = $norms['price'][$key];
                    $goods_norms->weight        = $norms['weight'][$key];
                    $goods_norms->stock         = $norms['stock'][$key];
                    $goods_norms->save();
                }
            } else {
