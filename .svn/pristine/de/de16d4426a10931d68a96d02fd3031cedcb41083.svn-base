...
@foreach($data as $key => $item)
    <li  class="@if($item['serviceTimesCount'] < 1 && $item['storeType'] != 1)x-rest @endif c-bgfff">
        <div class="item-link item-content">
            <div class="item-media"><a href="{{u('Seller/detail',['id'=>$item['id'],'urltype'=>1])}}"><img src="{{formatImage($item['logo'],100,100)}}" onerror='this.src="{{ asset("wap/community/newclient/images/no.jpg") }}"' width="73"></a></div>
            <div class="item-inner">
                <div class="item-title-row f16"  onclick="$.href('{{ u('Seller/detail',['id'=>$item['id'], 'urltype'=>2]) }}')" >
