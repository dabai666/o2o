...
class GoodsService extends \YiZan\Services\GoodsService {
	...
	public static function getSystemGoodsById($id) {
        $goods = Goods::where('goods.id', $id)
                      ->select('goods.*') 
                      ->with('seller', 'cate', 'norms', 'goodsProcessingCharges')
                      ->first();
     ...
     * @return array   创建结果
     */
    public static function systemSave( $sellerId, $sellerType, $type, $staffIds, $name, $price, $cateId, $brief, $images, $duration, $unit, $stock, $buyLimit, $norms, $status, $sort = 100, $systemTagListPid, $systemTagListId,$systemGoodsId = 0,$goodsSn,$brandId=0,$originalPrice=0,$salePrice=0,$weight=0,$name1=array(),$price1=array()) {
...
//                    $goods_norms->stock         = $norms['stocks'][$key];
            ...
            if($type == Goods::SELLER_GOODS) {
                ...
                foreach($norms['prices'] as $k => $v){
                    if($salePrice > $v){
                        $result['code'] = 28213;//商品规格原价要比促销价高
                        return $result;
                    }
                }
    ...
    public static function systemUpdate($id, $sellerId, $sellerType, $type, $staffIds, $name, $price, 
        $cateId, $brief, $images, $duration, $unit, $stock, $buyLimit, $norms, $status, $sort = 100, $systemTagListPid, $systemTagListId,$brandId=0,$originalPrice=0,$salePrice=0,$weight=0,$name1=array(),$price1=array(),$id1=array()) {
//        var_dump($originalPrice);die;
//        $price = doubleval($price);
        $result = array(
            'code'  => self::SUCCESS,
            'data'  => null,
