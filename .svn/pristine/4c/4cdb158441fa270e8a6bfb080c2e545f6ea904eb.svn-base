            ...
            <div class="list-block media-list y-ddlistblock">
                <ul>
                    ...
                    @foreach($v['orderGoods'] as $item)
                        <li onclick="$.herf('{{ u('Order/detail',['id'=>$v['id']]) }}')">
                                ...
                                <div class="item-inner f12">
                                    <div class="item-title-row">
                                        <div class="item-title">
                                            {{$item['goodsName']}}
                                            @if($item['goodsNorms'])
                                            <div class="item-after c-gray">
                                                规格：{{$item['goodsNorms']}}
                                            </div>
                                            @endif
                                            @if($item['processName'])
                                            <div class="item-after c-gray">
                                                加工方式：{{$item['processName']}}
                                            </div>
                                            @endif
                                        </div>
                                        <div class="c-gray">
                                        ...
                                        <div class="item-after">
                                            @if((float)$item['salePrice'] <= 0)
                                                <p>￥{{($item['price'] + $item['processPrice']) * $item['num']}}</p>
                                            @else
                                                <p>￥{{($item['salePrice'] + $item['processPrice']) * $item['num']}}</p>
                                            @endif
                                            @if($item['salePrice'] > 0)
                                                <!--<del class="c-gray">￥{{($item['price'] + $item['processPrice']) * $item['num']}}</del>-->
                                            @endif
                                        </div>
                                    </div>
                                    <!--@if($item['goodsNorms'])
                                        <div class="item-title-row c-gray">
                                            <div class="item-title">{{$item['goodsNorms']}}</div>
