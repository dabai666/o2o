...
class  OrderService extends \YiZan\Services\OrderService
{
    ...
    public function sale_statistics($sellerId){
//        self::endOrder();
...
//            ORDER_REFUND_ADMIN_REFUSE, //后台拒绝
        ...
        foreach ($list as $key => $val) {
            $data['orders'][$key] = [
                ...
                'sellerAddress' => $val['seller']['address'],//查看退款申请
            ];
    ...
    public static function getOrderById($sellerId, $staffId, $orderId)
    {
      ...
      if ($data) {
          ...
          $data['sellerSendWay'] =  $data['seller']['sendWay'];
          unset($data['seller']);
      }
      return $data;
    }
    ...
    public static function updateOrder($sellerId, $staffId, $id, $status, $remark,$express = ''){
        ...
        $updateStatus = [
            ...
            '5' => ORDER_STATUS_CALL_SYSTEM_SEND,
            '6' => ORDER_STATUS_AFFIRM_SELLER,
        ];
        ...
        if ($status == 2) {
            ...
            if($order->is_all == 1)
            {
                ...
                if($send_type == 1 && !$order->IsCashOnDelivery){
                    $data['status'] = ORDER_STATUS_SYSTEM_SEND;
                    //看看自动分配是否有人
                    if($order->order_type == 2 || ($order->send_way == 1 && $order->order_type == 1))
                    {
                        ...
                        if($newstaffId > 0){
                            ...
                            $data['seller_staff_id'] = $newstaffId;
                        }
                    }
                }
            }
        ...
        if ($status != 1 && $status != 2 ) {
            ...
            if($order['staff']['sellerId'] == 0 && $updateStatus[$status] == ORDER_STATUS_CANCEL_SELLER){
