...
@section('show_top')
    <header class="bar bar-nav y-barnav">
		<a class="button button-link button-nav pull-left external"  href="@if(!empty($nav_back_url) && strpos($nav_back_url, u('Coupon/usepromotion')) === false && strpos($nav_back_url, u('Goods/detail')) === false ){{$nav_back_url}}@else{{ u('GoodsCart/index') }} @endif"  data-transition='slide-out'>
                    ...
                    <ul>
                            ...
                            <a href="#" class="item-link item-content">
                                <div class="item-inner">
                                    @foreach($data as $val)
                                        ...
                                        if(!empty($val['norms']))
                                        {
                                            $val['price'] = $val['norms']['salePrice'] > 0 ? $val['norms']['salePrice'] : $val['norms']['price'];
                                        }
                                        if(!empty($val['goodsProcessingCharges'])){
                                            $val['price'] = $val['price'] + $val['goodsProcessingCharges']['price'];
                                        }
                                        ...
                                        @if($val['normsId'])
                                            <div class="item-title f12 c-gray">规格：{{$val['norms']['name']}} &nbsp;￥{{$val['norms']['price']}}</div>
                                        @else
                                            @if($val['goods']['model'])
                                                <div class="item-title f12 c-gray">{{$val['goods']['model']}}</div>
                                            @else
                                                <div class="item-title f12 c-gray">无</div>
                                            @endif
                                        @endif
                                        @if($val['goodsProcessingCharges'])
                                                <div class="item-title-row f14">
                                                    <div class="item-title"><span class="item-title f12 c-gray">加工：{{$val['goodsProcessingCharges']['name']}} &nbsp;￥{{$val['goodsProcessingCharges']['price']}}</span></div>
                                                    <!--<div class="item-after c-black"><span class="c-gray">x{{$val['num']}}</span><span class="pl20">￥{{$val['goodsProcessingCharges']['price']*$val['num']}}</span></div>-->
                                                </div>
                                        @endif
                                    @endforeach
                                </div>
                        ...
                        <li >
                            <a href="#" class="item-link item-content">
                                <div class="item-inner">
                                    <div class="item-title-row f14">
                                        <div class="item-title">{{ $v['goods']['name'] }} 加工:{{ $v['goodsProcessingCharges']['name'] }}</div>
                                        <div class="item-after c-black"><span>￥{{ $v['goodsProcessingCharges']['price'] or '0.00'}}</span></div>
                                    </div>
                                </div>
                            </a>
                        </li>
                            ...
                            <a href="#" class="item-link item-content">
                                <div class="item-inner">
                                    <div class="item-title-row f12">
                                        <!--$oldPriceTotal-->
                                        <div class="item-title c-gray">原价<span class="c-black" id="oldPriceTotal">￥{{$fee['totalFee'] or '0.00'}}</span> - 共优惠<span class="c-black" id="cutDownTotal">￥{{$cutDownTotal or '0.00'}}</span></div>
