            ...
            <li class="item-content p0">
                <div class="item-inner pr10 pl10">
                    <div class="item-title-row f14">
                        <div class="item-title">消费码：{{ $data['authCode'] }}</div>
                        @if($data['orderStatusStr'] == '交易关闭')
                        <div class="item-after c-gray">已失效</div>
                        @elseif($data['authCodeUse'] == -1)
                        <div class="item-after c-gray">已使用</div>
                        @elseif($data['authCodeUse'] == 1)
                        <div class="item-after">未使用</div>
                        @endif
                    </div>
                </div>
            </li>
                    ...
                    <li class="item-content">
                        <div class="item-inner pr10">
                            ...
                            <div class="item-title-row">
                                ...
                                @if($val['salePrice'] > 0)
                                <span class="y-delgrid mr15 c-gray">￥{{ number_format(($val['price'] + $val['processPrice'])*$val['num'], 2) }}</span> <!-- 原价 -->
                                <!--<div class="item-after"><span>￥{{ number_format($val['salePrice'], 2) }}</span></div> <!-- 成交价 -->
                                @else
                                <span class="y-delgrid mr15 c-gray"></span> <!-- 原价 -->
                                <div class="item-after"><span>￥{{ number_format(($val['price'] + $val['processPrice'])*$val['num'], 2) }}</span></div> <!-- 成交价 -->
                                @endif
                            </div>
                            @if($val['goodsNorms'])
                                <div class="item-title f12 c-gray">规格:{{$val['goodsNorms']}} &nbsp;{{$val['price']}}</div>
                            @endif
                            @if($val['processId'])
                                    <div class="item-title-row">
                                        <div class="item-title f12 c-gray">加工方式:{{$val['processName']}} &nbsp;{{$val['processPrice']}}</div>
                                        <!--<span class="mr15 c-gray">x{{$val['num']}}</span>
                                        <span class="y-delgrid mr15 c-gray"></span> <!-- 原价 -->
                                        <!--<div class="item-after"><span>￥{{ number_format($val['processPrice']*$val['num'], 2) }}</span></div> <!-- 成交价 -->
                                    </div>
                                @endif
                            @endforeach
                        </div>
                            ...
                            <div class="item-inner f12">
                                ...
                                @if($goods['goodsNorms'])
                                    <div class="item-title-row c-gray">
                                        <div class="item-title">规格:{{$goods['goodsNorms']}}</div>
                                ...
                                @if($val['processId'])
                                    <div class="item-title-row">
                                        <div class="item-title">加工方式:{{$val['processName']}}</div>
                                        <span class="mr15 c-gray">x{{$val['num']}}</span>
                                        <span class="y-delgrid mr15 c-gray"></span> <!-- 原价 -->
                                        <div class="item-after"><span>￥{{ number_format($val['processPrice']*$val['num'], 2) }}</span></div> <!-- 成交价 -->
                                    </div>
                ...
                @if($data['promotionIsShow'] != 1)
                    ...
                    $.alert($("#x-tkmodaltext").html(), $("#x-tkmodaltitle").html(), function () {
                        if (window.App){
							...
							var share_data = {
								share_content:'{!! $activity['shareContent'] !!}',
								share_imageUrl:"{!! $activity['shareLogo'] !!}",
								share_url:'{!! $link_url !!}',
								share_key:'',
								share_title:'{{$activity['shareTitle']}}' ,
								custom_type: custom_type,								
