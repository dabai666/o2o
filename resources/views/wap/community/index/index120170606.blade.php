            ...
            @if($module)
                <ul class="discount">
                    @foreach($module as $k => $v)
                        <li style="display: flex;display:-webkit-flex;justify-content: space-between;align-items: center;" class="flex justify-content align-item" data-type="{{ $v['type'] }}" data-url="@if(!strstr($v['url'],'http://') || !strstr($v['url'],'https://')) {{ 'https://'.$v['url'] }} @else{!! 'https://'.$v['url'] !!} @endif ">
                            <div style="padding:0 0 0 1rem;">
                                <a href="{{ $v['url'] }}" external>
                                    <p class="colorange" style="color:@if($v['type'] == 1) #089ce3 @elseif($v['type'] == 2) #fe9200 @elseif($v['type'] == 3) #9bbb2a @else #ee7457 @endif; font-size: 18px;">{{ $v['name'] }}</p>
                                    <span style="color: rgb(165,157,155);">{{ $v['produce'] }}</span>
                                </a>
                            </div>
                            <div style="padding:.5rem 0;"><a href="{{ $v['url'] }}"><img src="{{ $v['image'] }}"></a></div>
                        </li>
                    @endforeach
                </ul>
                @endif
                ...
                @if($brandAdv)
                    ...
                    @if(count($data['notice']) > 0)
                        <ul class="x-advertising clearfix" style="background:none;">
                            @foreach($data['notice'] as $k=>$value)
                                @if($k < 2)
                                    <li style="padding:0 0.5%;position:static;"><a href="{{$value['url']}}" class="br pageloading"><img src="{{ formatImage($value['icon'],293) }}" style="height:100px;"></a></li>
                                @endif
                            @endforeach
                        </ul>
                    @endif
