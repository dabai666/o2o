        ...
        $(document).on('click','.x-hislst2 li',function () {
            ...
            $.post("{{ u('Index/relocation2') }}",data,function(res){
                if(res.code == 1){
                    ...
                    if(type == 1){
                        $.router.load("{{u('UserCenter/addressdetail',['SetNoCity'=>Input::get('SetNoCity')])}}&address="+data.address+"&mapPointStr="+data.mapPointStr+"&cityId="+res.cityId+"&gps=1&cartIds="+cartIds, true)
                    }else if(type == 2){
						...
						$.router.load("{{u('UserCenter/detailreg')}}?city="+data.address+"&mapPointStr="+data.mapPointStr+"&cityId="+res.cityId, true)
					}
                }
            },"json");
        });
