...
@section('show_top')
    <header class="bar bar-nav">
        <a class="button button-link button-nav pull-left" onclick="javascript:$.back();" data-transition='slide-out'>
...
@section('content')
    <div class="bar bar-header-secondary y-twonobor">
        <div class="buttons-tab y-couponsnav y-splistm y-noborder">
            <a href="tab1" class="tab-link button @if(empty($args['payitemId'])) active @endif" onclick="$.href('{{u('PropertyFee/index',['sellerId'=>$args['sellerId']])}}')">全部缴费</a>
            <a href="tab2" class="tab-link button y-sxbtn @if(!empty($args['payitemId'])) active @endif"><span>
