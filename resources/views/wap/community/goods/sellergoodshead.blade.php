...
<header class="bar bar-nav y-sjlistnav">
    <a class="button button-link button-nav pull-left pageloading" href="javascript:$.href('@if(!empty($nav_back_url) && strpos($nav_back_url, u('Goods/index')) === false && strpos($nav_back_url, u('Goods/comment')) === false && strpos($nav_back_url, u('Goods/detail')) === false ){{$nav_back_url}}@else @if(Input::get('activityId') && Input::get('index_flage')) {{ u('Activity/secsale',['index_flage'=>Input::get('index_flage'),'activityId'=>Input::get('activityId')]) }} @else {{ u('Index/index') }} @endif @endif')" data-transition='slide-out'>
    ...
    <a class="button button-link button-nav pull-right open-popup collect_opration" data-popup=".popup-about">
        ...
        @if($seller['isCollect'] == 1)
            <!-- 已收藏图片  -->
            <i class="icon collect iconfont c-red m0 on">&#xe654;</i>
            @else
                    <!-- 未收藏图片  -->
    ...
    <div class="list-block media-list y-sylist y-sjxq clear">
        <ul>
            <li>
                <a href="{{u('Seller/detail',['id'=>Input::get('id'),'showurl' => 1])}}" class="item-link item-content">
