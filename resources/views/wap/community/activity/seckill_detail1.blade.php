...
@section('content')
    ...
    @include('wap.community._layouts.base_cart_item')
            <!--商品详情页 start-->
    <div class="content infinite-scroll infinite-scroll-bottom"  data-distance="50" id="" style="">
        <!--banner start-->
        @if($data)
            @foreach($data as $k => $v)
                @foreach($v['goodsList'] as $kk => $vv)
                    <div>
                        <img _src="{{ formatImage($vv['image'],640) }}" src="{{ formatImage($vv['image'],640) }}" />
                    </div>
                    ...
                    <div style="margin-top: 10px;">
                        <div class="flex justify-content pd" style="width:100%;background-color:#fff;">
                            <div>
                                <h4>{{ $vv['name'] }}</h4>
                                <p class="font12">
                                    <span>剩{{ ($vv['stock'] - $vv['useStock']) }}份</span>
