...
@section('show_top')
    <header class="bar bar-nav">
        <a class="button button-link button-nav pull-left external"  href="@if(!empty($nav_back_url) && strpos($nav_back_url, u('Coupon/index')) === false){{$nav_back_url}}@else{{ u('UserCenter/index') }} @endif"  data-transition='slide-out'>
...
@section('content')
    ...
    <div class="bar bar-header-secondary">
        <div class="buttons-tab y-couponsnav">
            <a href="{{ u('Coupon/index',['status' => 0,'flage'=>$args['flage']]) }}" class="button @if($args['status'] != 1) active @endif pageloading">未使用</a>
            <a href="{{ u('Coupon/index',['status' => 1,'flage'=>$args['flage']]) }}" class="button @if($args['status'] == 1) active @endif pageloading">已失效</a>
        </div>
    </div>
