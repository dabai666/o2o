...
@section('right_content')
    ...
    <yz:form id="yz_form" action="save_sale_activity">
        ...
        <div id="bgimage-form-item" class="u-fitem clearfix " style="margin-left: 20px;">
		            <span class="f-tt">
		                 活动banner：
		            </span>
            <div class="f-boxr">
                <ul class="m-tpyllst clearfix">
                    <li id="bgimage-box" class="">
                        ...
                        @if(!empty($data['image']))
                            <input type="hidden" data-tip-rel="#bgimage-box" name="image" id="bgimage" value="{{$data['image']}}">
                        @else
                            <input type="hidden" data-tip-rel="#bgimage-box" name="image" id="bgimage" value="">
                        @endif
                    </li>
            ...
            <div class="f-boxr">
                <ul class="m-tpyllst clearfix">
                    <li id="image-box" class="">
                        ...
                        @if(!empty($data['shopImage']))
                            <input type="hidden" data-tip-rel="#image-box" name="shopImage" id="image" value="{{$data['shopImage']}}">
                        @else
                            <input type="hidden" data-tip-rel="#image-box" name="shopImage" id="image" value="">
                        @endif
                    </li>
            ...
            <div class="f-boxr">
                <ul class="m-tpyllst clearfix">
                    <li id="icon-box" class="">
                        <a id="img-preview-icon" class="m-tpyllst-img" href="javascript:;" target="_self">
                            @if(!empty($data['icon']))
                        ...
                        @if(!empty($data['icon']))
                            <input type="hidden" data-tip-rel="#icon-box" name="icon" id="icon" value="{{$data['icon']}}">
                        @else
                            <input type="hidden" data-tip-rel="#icon-box" name="icon" id="icon" value="">
                        @endif
                    </li>
        ...
        <yz:fitem name="price" label="商品价格"></yz:fitem>
        <yz:fitem name="salePrice" label="促销价格"></yz:fitem>
        <yz:fitem name="model" label="型号"></yz:fitem>
        <yz:fitem name="weight" label="重量"></yz:fitem>
        <yz:fitem name="goodsUnit" label="单位"></yz:fitem>
                    ...
                    <ul class="p-sellerlist">
                        @foreach($sellerLists as $key => $value)
                            <li>
                                ...
                                商品量: <input type="text" value="{{$sellerLists[$key]['totalStock']}}" name="stocks[{{$key}}]" class="u-ipttext addseller totalStock" data-sellerId='{{$key}}'>
    ...
    <script type="text/javascript">
        $(function(){
            ...
            $("input.p-addbtn").click(function(){
                ...
                var sellerIds = [];
//                var sellerTotalstocks = [];
                $.each($("#useSellerList ul li"), function(k, v){
                    sellerIds[k] = $(this).find('input.addseller').attr('data-sellerId');
//                    sellerTotalstocks[$(this).find('input.addseller').attr('data-sellerId')] = $(this).find('input.totalStock').val();
                });
                /*if(sellerTotalstocks != ''){
                    sellerTotalstocks = $.grep(sellerTotalstocks, function(n) {return $.trim(n).length > 0;});
                }*/
                saveData.form = $("#yz_form").serializeArray();
                saveData.sellerIds = sellerIds;
                saveData.brief = $("iframe").contents().find("body").html();
//                saveData.sellerTotalstocks = sellerTotalstocks;

                $.post("{{ u('Activity/save_sale_data') }}", saveData, function(res){
//                    alert(1);return false;
                    window.location.href = "{{ u('Activity/addSeller', ['type' => 8]) }}";
                });
            });
            ...
            $(".p-sellerlist li i.fa").click(function(){
                ...
                //异步请求
                $.post("{{ u('Activity/deleteSellerIds') }}", {'id':id,'type':"{{$args['type']}}"}, function(res){
                    //动画移除
                    if(res == 1){
