	...
	<yz:list>
		<search method="get">
			<row>
                ...
                <item label="会员标签">
                    <yz:select name="userTag" options="$userTagInfo" valuefield="id" textfield="name" selected="$search_args['userTag']"></yz:select>
                </item>
                <btn type="search"></btn>
			</row>
		</search>
		...
		<table checkbox="1">
			<columns>
				...
				<column label="名称" align="left" code="name">
					<p>
                        {{ $list_item['name'] }}
                ...
                <column label="会员标签" align="" width="150" code="name">
                    <p>
                        @if($list_item['shoptype'])
                            @foreach(explode(',',$list_item['shoptype']) as $v)
                            {{ $userTag[$v] }}&nbsp;
                            @endforeach
                        @else
                            无
                ...
                <!-- <column label="注册时间" align="left" width="140">
					<p><b>注 册 I P:</b>{{ $list_item['regIp'] }}</p>
					 <p>{{ $list_item['regCity'] != "" ? "<b>注册城市:</b>".$list_item['regCity'] .'-' .$list_item['regProvince'] : "未获取到注册城市"}}</p>
					<p>{{ yztime($list_item['regTime'])  }}</p>
				</column> -->
                <!--<column label="登录信息" align="left"  width="150">
					<p>{{ $list_item['loginIp']   != "" ? "登 录 I P:". $list_item['loginIp'] : "未获取到最后登录IP地址"}}</p>
					 <p>{{ $list_item['loginCity'] != "" ? "登录城市:".$list_item['loginCity']['name'] .'-' .$list_item['loginProvince']['name'] : "未获取到最后登录城市"}}</p>
					<p>{{ $list_item['loginTime'] != "" ? "登录时间:". yztime($list_item['loginTime']) : "未获取到最后登录时间"}}</p>
				</column>-->
