...
@yizan_begin
	<yz:form id="yz_form" action="update"> 
		<div class="udb_br">		
			...
			@if($data['detail'])
				<yz:fitem label="店铺名" >
					<p>{{ $data['detail']['shopname'] }}</p>
				</yz:fitem>
				<yz:fitem label="负责人" >
					<p>{{ $data['detail']['personcharge'] }}</p>
				</yz:fitem>
				<yz:fitem label="会员标签" >
					<p>
			...
			@if($userAddressInfo)
				<yz:fitem label="详细地址" >
					<p>{{ $userAddressInfo['province']['name']." ".$userAddressInfo['city']['name']." ".$userAddressInfo['area']['name']." ".$userAddressInfo['address'] }}</p>
				</yz:fitem>
			@endif
			<div class="u-rtt f3f6fa">
