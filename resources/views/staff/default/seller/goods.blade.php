...
@section('show_top')
    <header class="bar bar-nav">
        ...
        @if($cate['autoType'] == 0)
        <a href="javascript:;" onclick="JumpURL('{{u('Seller/systemgoods',['type' => 1,'tradeId'=>(int)Input::get('id')])}}','#seller_systemgoods_view',2)" class="button external pageloding button-link button-nav f_r" data-popup=".popup-about">
            添加商品
        </a>
        @endif
...
@section('content')
    <div class="p075" style=" padding-bottom:0!important; overflow:hidden;">
        <input type="text" name="keywords" id="keywords_word" onkeydown="$.keywords_goods({{$status}},{{$id}},1);" class="icon iconfont tc search-box-input" placeholder="&#xe63e; 请输入想要搜索的商品" value="{{Input::get("keywords")}}"/>
    </div>
    <div class="management-editor plr085 clearfix">
        ...
        @if($goods && ($goods[0]['cate']['autoType'] == 0))<span  class=" f_r focus-color-f" onclick="$.sellerEditor('seller_editor-but_goods')" id="seller_editor-but_goods">编辑</span>@endif
    </div>
