	    ...
	    <div class="list-block media-list m0 y-checkboxjs">
	        <ul id="sendWayList">
	            <li class="item-content @if(in_array(1,$data['sendWay'])) active @endif" data-id="1">
	                <div class="item-inner f_5e">
	                    <div class="item-title-row">
	                        <div class="item-title f13 name">送货上门</div>
	        ...
	        <ul id="sendTypeList">
	            ...
	            <li>
	                <div class="p10 f_999 f12">
	                    ...
	                    <!-- <p>1、配送托管：配送运力全部由平台提供，按次收费，每单配送服务费为 {{$system_send_staff_fee or '0'}} 元，平台自动分配平台配送员上门取货并完成配送。</p> -->
	                </div>
	            </li>
	        </ul>
	    </div>
...
<script type="text/javascript">
	$(function(){
	    //消费方式
	    $(document).on("click",".y-xffstcmain",function(){
	        $(".y-modal-overlay").addClass("y-modal-overlay-visible");
	        $("#y-xffstcmain").addClass("y-modal-in").removeClass("y-modal-out");
	    })
	    //配送服务
	    $(document).on("click",".y-psfwtcmain",function(){
	        $(".y-modal-overlay").addClass("y-modal-overlay-visible");
	        $("#y-psfwtcmain").addClass("y-modal-in").removeClass("y-modal-out");
	    })
	    //关闭弹窗
	    $(document).on("click",".y-modal-overlay",function(){
	        $(".y-modal-overlay").removeClass("y-modal-overlay-visible");
	        $(".y-actions-modal").removeClass("y-modal-in").addClass("y-modal-out");
	    })
	    //弹窗复选按钮
	    $(document).on("click",".y-checkboxjs li",function(){
	    ...
	    //弹窗单选按钮
	    $(document).on("click",".y-radiojs li.item-content",function(){
	        $(this).addClass("active").siblings(".item-content").removeClass("active");
	    })
	    //金额处理
	    $(".moneyToFixed").focus(function(){
	    	if(this.value <= 0)
	    		this.value = '';
	    });
	    $(".moneyToFixed").blur(function(){
	    	if(this.value < 0)
        ...
        //消费方式
        $(document).off("click","#sendWayBtn");
        $(document).on("click", "#sendWayBtn", function(){
        	var text  = '';
        	var val   = '';
        	var first = true;
        	//隐藏配送服务
        	...
        	$("ul#sendWayList li").each(function(k, v){
        		if($(this).hasClass('active'))
        		{
        			if(first)
        			{
         ...
         //配送服务
        $(document).off("click","#sendTypeBtn");
        $(document).on("click", "#sendTypeBtn", function(){
        	var text  = '';
        	var val   = '';
        	$("ul#sendTypeList li").each(function(k, v){
        		if($(this).hasClass('active'))
        		{
    				text = $(this).find('.name').text();
    				val  = $(this).data('id');
