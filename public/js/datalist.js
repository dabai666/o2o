	...
	$(document).on('click','.table-status',function(){
		...
		$.post(SITE_URL + '/' + controller + '/updateStatus', query, function(result){
			if(result.status == 1){
				...
				if(query.val == 0){
					obj.removeClass('fa-check text-success table-status1').addClass('fa-lock table-status0').attr('status',1).attr('title',title).html('下架');
				}else{
					obj.removeClass('fa-lock table-status0').addClass('fa-check text-success table-status1').attr('status',0).attr('title',title).html('上架');
				}
